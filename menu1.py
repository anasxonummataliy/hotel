...
class Menu(BaseJSON):
    def __init__(self, user_id):
        ...
        self.profil = {"id": user_id, "ism": "Bekzod", "lvl": "Gold"}
    ...
    def show_available_rooms(self):
        for i, room in enumerate(self.rooms, 1):
            if room["status"] == "Available":
                print(
                    f"{i}. Xona {room['number']} | {room['type']} | {room['price']} so'm"
                )
    ...
    def mening_bronlarim(self):
        ...
        if not user_bookings:
            ...
            for b in user_bookings:
                ...
                print(
                    f"- Xona {room['number']} | {room['type']} | {b['check_in']} -> {b['check_out']} | {b['total_price']} so'm | {b['status']}"
                )
