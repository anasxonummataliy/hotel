...
class Hotel:
    ...
    def create_booking(self):
        rooms = get_data(self.rooms_file)
        bookings_list = get_data(self.bookings_file)
        ...
        while True:
            ...
            try:
                ...
                if check_out_date <= check_in_date:
                    print(
                        colored(
                            "❌ Ketish sanasi kelish sanasidan katta bo'lishi kerak!",
                            "red",
                        )
                    )
                    continue
        ...
        save_data(self.bookings_file, bookings_list)
    ...
    def my_bookings(self):
        ...
        user_bookings = [b for b in bookings_list if b["user_id"] == self.user_id]
        ...
        if cancel.lower() == "ha":
            self.cancel_booking(bookings_list)

    def cancel_booking(self, bookings_list):
        booking_id = input(
        ...
        for booking in bookings_list:
            if booking["id"] == booking_id and booking["user_id"] == self.user_id:
    ...
    def show_profile(self):
        ...
        for user in users:
            if user["id"] == self.user_id:
                ...
                print(
                    colored("Telefon  >>> ", "yellow")
                    + colored(user["phone"], "magenta")
                )
                print(
                    colored("Login    >>> ", "yellow")
                    + colored(user["login"], "magenta")
                )
                break
